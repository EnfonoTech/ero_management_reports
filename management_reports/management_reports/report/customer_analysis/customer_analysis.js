frappe.query_reports["Customer Analysis"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
	...
	formatter: function (value, row, column, data, default_formatter) {
		...
		if (column.fieldname === "share" && data) {
			if (data.share >= 10) {
				value = `<span style="color: #0f3460; font-weight: 600">${value}</span>`;
			}
		}
		if (column.fieldname === "revenue" && data && data.revenue >= 50000) {
